			...
			<div class="min-h-full w-10/12 py-14 z-20">
				...
				<div class="flex justify-between items-center">
					<div class="flex flex-col">
						<p class="text-2xl font-bold">Student Management</p>
						<p class="text-sm text-slate-500">Review and manage student accounts</p>
					</div>
				</div>
				...
				<div class="flex flex-col mt-5 gap-2 pb-24">
					...
					<div class="grid w-full justify-items-end mt-5">
						<div class="flex w-full gap-2 border p-4 bg-white rounded-md items-end">
							...
							<div class="flex flex-col gap-1 w-1/4">
								...
								<select id="course-filter" class="border rouded-sm border-slate-300 bg-slate-100 py-1 px-2 outline-1 outline-blue-500 text-neutral-700">
					...
					<div class="flex flex-col gap-2 px-4 py-2 border bg-white rounded-md mt-5">
						<div class="flex items-center justify-between py-2">
							<p class="p-2 font-semibold">Student Summary</p>
							<div class="flex gap-2 items">
								<form id="import-form" method="POST" action="<?php echo URLROOT; ?>/student/import" enctype="multipart/form-data">
									<label for="excel-file" id="import-btn" class="flex gap-1 items-center bg-blue-700 text-white rounded-md px-4 py-1 h-max">
								...
								<a href="<?php echo URLROOT;?>/student/add">
								...
								<form action="<?php echo URLROOT;?>/user/multiple_delete/student" method="POST" id="multiple-drop-form" class="hidden">
							...
							<tbody>
								...
								?>
									<tr class="border-b border-slate-200">
										...
										<td class="hidden"><?php echo $row->type; ?></td>
										...
										<td class="text-center">
											...
											<?php if($row->status == 'active' || $row->status == 'closed' || $row->status == 'blocked'): ?>
												<a class="hover:text-blue-700" href="<?php echo URLROOT.'/student/records/'.$row->id ; ?>">view</a>
											<?php endif; ?>

											<?php if($row->status == 'closed'): ?>
												<a id="open-account-btn" class="hover:text-blue-700" title="open account" href="<?php echo URLROOT.'/user/open/student/'.$row->id;?>">activate</a>
											<?php endif; ?>

											<?php if($row->status == 'blocked'): ?>
												<a id="unblock-account-btn" class="hover:text-blue-700" href="<?php echo URLROOT.'/user/unblock/student/'.$row->id;?>" title="unblock account">unblock</a>
											<?php endif; ?>

											<?php if($row->status == 'closed' || $row->status == 'blocked' || $row->status == 'declined'): ?>
												<a id="delete-btn" class="text-red-500" href="<?php echo URLROOT.'/user/delete/student/'.$row->id ; ?>">delete</a>
											<?php endif; ?>
											...
											<?php if($row->status == 'active'): ?>
												<a id="close-account-btn" class="text-red-500" title="close account" href="<?php echo URLROOT.'/user/close/student/'.$row->id;?>">close</a>
		...
		<div class="flex justify-center w-full h-max">
			<div class="flex flex-col w-10/12 pt-10 pb-20">
				<div class="flex flex-col gap2 w-full">
					<p class="text-2xl font-bold">Student Details<span class="text-sm font-normal" id="request-id"></span></p>
				...
				<div class="flex flex-col gap2 w-full mt-6">
					<table class="w-full table-fixed">
						...
						<tr>
							<td class="hover:bg-slate-100 text-slate-500 p-1 pl-2" width="20">Year level</td>
...
<script>
	<?php
		require APPROOT.'/views/user/student/student.js';
	?>
</script>