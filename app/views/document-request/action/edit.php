				...
				<div class="w-10/12">
					...
					<form method="POST" action="<?php echo URLROOT;?>/academic_document/add" enctype="multipart/form-data">
						...
						<?php
							...
							if(isset($data['input-details']->is_tor_included)) {
								if($data['input-details']->is_tor_included) {
							...
							if(isset($data['input-details']->is_diploma_included)) {
								if($data['input-details']->is_diploma_included) {
							...
							if(isset($data['input-details']->is_gradeslip_included)) {
								if($data['input-details']->is_gradeslip_included) {
							...
							if(isset($data['input-details']->is_ctc_included)) {
								if($data['input-details']->is_ctc_included) {
							...
							if(isset($data['input-details']->is_diploma_included)) {
								if($data['input-details']->is_diploma_included) {
									...
									$isOthersChecked['for-hidden-input-value'] = $data['input-details']->other_requested_document;	
								}
							}

							//beneficiary
							$isBeneficiary = ['yes'=>'', 'no' => '', 'for-hidden-input' => 'hidden'];
							if(isset($data['input-details']->is_RA11261_beneficiary)) {
								if($data['input-details']->is_RA11261_beneficiary == 'yes') {
									$isBeneficiary['yes'] = 'selected';	
									$isBeneficiary['for-hidden-input'] = '';
								}elseif($data['input-details']->is_RA11261_beneficiary == 'no') {
									$isBeneficiary['no'] = 'selected';	
								}
							}

							$purposeOfRequest = '';
							if(isset($data['input-details']->purpose_of_request)) {
								if($data['input-details']->purpose_of_request) {
									$purposeOfRequest = $data['input-details']->purpose_of_request;	
								}
							}
